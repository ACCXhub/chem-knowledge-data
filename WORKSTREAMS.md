...
| `packages/thermochemistry/` | **COMPLETE / LOCKED** | `chatgpt-web-thermochemistry`（已完成） | `thermochemistry-v0.1.0` 已标记 `READY_FOR_CONSOLIDATION`；54 条 typed records，作为 consolidation 只读输入 |
...
- `packages/inorganic/` v1.0.1、`packages/organic/` v0.2.0、`packages/structural_chemistry/` v1.0.2、`packages/thermochemistry/` v0.1.0 已可作为冻结输入参与统一。
...
- `packages/thermochemistry/` 管理 phase facts、phase-specific thermochemistry、phase-transition enthalpy 与 educational bond-enthalpy references；它不拥有 Species/Reaction identity。
