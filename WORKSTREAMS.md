...
这个文件用于多聊天并行开发时声明包级 ownership。开始修改任何 `packages/*` 前先检查这里，并结合 `coordination/claims/` 与包内 `STATUS.md` 判断当前写入边界。

| Package | Status | Active owner | Write boundary |
|---|---|---|---|
| `packages/inorganic/` | **COMPLETE / LOCKED** | `chatgpt-web-inorganic`（已完成） | v1.0.1 已完成最终复核并标记 `READY_FOR_CONSOLIDATION`；作为 consolidation 只读输入，后续变更按新数据集 revision 处理 |
...
- `packages/inorganic/` v1.0.1 作为 `READY_FOR_CONSOLIDATION` 只读完成输入；后续跨包对齐在 consolidation 中完成，不回写无机源包。
