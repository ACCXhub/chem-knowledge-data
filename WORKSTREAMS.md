...
| `packages/structural_chemistry/` | **ACTIVE / LOCKED** | `chatgpt-web-structural-chemistry` | 负责高中结构化学 / 物质结构与性质知识；不拥有 canonical `structure_id` |
...
- `packages/structural_chemistry/` 是高中“结构化学”知识包，负责原子结构、化学键、VSEPR、杂化、晶体与结构—性质教学关系；只引用 `structure_registry` 的已发布结构身份。
