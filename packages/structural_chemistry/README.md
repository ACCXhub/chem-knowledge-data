...
本包负责高中化学“物质结构与性质 / 结构化学”知识数据，作为 `inorganic`、`organic`、`structure` 与 `consolidated` 的只读上层教学知识包。
...
- 原子核外电子排布与轨道教学知识；
- 化学键、键极性、σ/π 键、配位键等结构概念；
- VSEPR / 杂化 / 分子空间构型 / 分子极性教学数据；
- 分子间作用力与结构—性质关系；
- 分子晶体、共价晶体、离子晶体、金属晶体等晶体教学模型；
- 配位化学基础与典型高中案例；
- 面向高中课程的 topic / priority / misconception / relation 投影。

本包不拥有：

- SMILES、InChI、InChIKey、结构规范化身份：由 `packages/structure` 负责；
...
- `data/atomic_structure.jsonl`：原子电子排布与周期性教学数据。
- `data/molecular_geometry.jsonl`：VSEPR / 杂化 / 空间构型案例。
- `data/crystal_models.jsonl`：晶体类型与典型案例。
- `data/coordination.jsonl`：配位化学基础案例。
- `data/structure_property_relations.jsonl`：结构—性质教学关系。
- `validation/validate.py`：零第三方依赖的基础一致性校验。
...
当前为 foundation 阶段：先覆盖高中主干概念、代表性模型与消费契约；后续按上游 stable species / structure 增量补充链接和更多案例。
