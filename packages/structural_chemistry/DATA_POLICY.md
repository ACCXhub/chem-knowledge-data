...
- `structure_id`：只引用 `packages/structure` 已发布 ID。
...
- `packages/structure`：本仓库可计算 Structure identity 的唯一直接引用源。
...
foundation 数据可以在本包内以 `reviewed` 状态积累；只有满足 schema、引用和事实复核 gate 后才升级为 `published` release。
